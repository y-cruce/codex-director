...
The Codex plugin is installed and its quota is effectively unlimited. For any task that involves reading code to understand current behavior, debugging, implementing a change, or reviewing a diff, load the `codex-director` skill first and follow its process to dispatch the work to Codex. Do only the brief writing, result judging, and decision making yourself. When the fallback `codex-worker` agent is needed, set `model` to `opus`.
```
